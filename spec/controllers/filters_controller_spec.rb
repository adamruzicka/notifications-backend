...
RSpec.describe FiltersController, type: :controller do
  describe 'GET #show' do
    ...
    it 'returns 404 if the endpoint does not exist' do
      ...
      expect(error).to eq("Could not find Endpoint with 'id'=#{endpoint.id + 1}")
    end
    ...
    it 'succeeds if the endpoint exists but does not have an endpoint' do
    ...
    it 'returns 404 if the endpoint exists but belongs to another user' do
      ...
      expect(response).to have_http_status(:not_found)
      error = JSON.parse(response.body)['errors']
      expect(error).to eq("Could not find Endpoint with 'id'=#{endpoint.id}")
    end
  end
end
